...
#[wasm_bindgen]
pub struct SccGroups1 {
    #[wasm_bindgen(skip)]
    pub inner: Rc<Vec<Vec<graph::NodeIndex>>>,
}

#[wasm_bindgen]
pub struct SccGroup1 {
    #[wasm_bindgen(skip)]
    pub inner: Rc<Vec<Vec<graph::NodeIndex>>>,
...
impl SccGroups1 {
    pub fn get(&self, index: usize) -> JsValue {
        match self.inner.len() > index {
            true => JsValue::from(SccGroup1::new(self.inner.clone(), index)),
            false => JsValue::undefined(),
        }
    }

    /// Create copy of SccGroups1 as `Array<Array<number>>`.
    /// This is a convenience method for workings with SccGroups1!
...
mod tests {
    ...
    fn new_test_scc_groups() -> SccGroups1 {
        SccGroups1::new(vec![
            vec![
                NodeIndex::new(0),
                NodeIndex::new(1),
    ...
    fn can_access_scc_groups() {
        ...
        let scc_group0: JsValue =
            js_sys::Reflect::apply(&scc_groups_get, &scc_groups, &js_array_0).unwrap();
        ...
        let scc_group0_item0: JsValue =
            js_sys::Reflect::apply(&scc_group0_get, &scc_group0, &js_array_0).unwrap();
